...
### 2026-05-07（v0.4.6）
- **資產加載與報錯修復**：
  - **音效 ID 替換**：更換了 4 個不相容或失效的音效 ID（揮擊、命中、金幣、魔法），改用更現代且可靠的公開音效，解決「Asset type does not match」報錯。
  - **NPC 視覺修復**：將 NPC 使用的失效 Mesh ID 更換為可靠的公開貓頭 Mesh (1635970808)，解決「could not fetch」報錯。
  - **尾巴加載邏輯增強**：同步將尾巴套用邏輯升級為 Accessory 模式優先，支援 13 位數以上的 UGC 資產 ID，並自動配置 `WaistCenterAttachment` 掛載點。
  - **虛擬配件定位優化**：在 `applyAccessory` 中細化了掛載點邏輯，根據配件名稱（Hood/Tail/Suit）自動選擇正確的 Attachment。
- **版本號更新**：升級至 `v0.4.6`。

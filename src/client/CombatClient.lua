...
local CatVisualData = require(ReplicatedStorage:WaitForChild("Shared").CatVisualData)
...
CombatClient.currentCatId = "whiteCat" :: string

-- ── 動態臉部系統 ──────────────────────────────────────────────────

local function updateFace(faceType: "idle" | "blink" | "combat")
	local char = localPlayer.Character
	if not char then return end
	
	local catHead = char:FindFirstChild("CatHeadShape")
	if not catHead then return end
	
	local decal = catHead:FindFirstChild("DynamicFace") :: Decal?
	if not decal then return end
	
	local visualInfo = CatVisualData.cats[CombatClient.currentCatId]
	if visualInfo and visualInfo.faces[faceType] then
		decal.Texture = visualInfo.faces[faceType]
	end
end

local function triggerCombatFace(duration: number)
	updateFace("combat")

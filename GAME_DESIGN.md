...
### 2026-05-05（v0.3.3）
- **主角外觀實作切換**：`CatAppearance.apply` 改為保留 Roblox 預設身體部位，不再呼叫全身 `Cat*` 形體替換。
- **套裝化落地**：改用「身體主色套色 + 貓耳 + 貓尾」配件式外觀，維持預設角色骨架與動畫相容。
- **風險收斂**：降低自訂骨架與預設 Animator 搶寫關節造成的戰鬥動畫衝突風險。
...
- **初始化防重複綁定**：`CombatClient.init()` 新增 `_initialized` 守衛，避免重生或重複載入導致事件多次綁定、音效重疊觸發。
- **攻擊手部動作修正**：揮擊動畫改為「右肩主揮擊 + 右肘彎折 + 左肩/左肘反向平衡」多關節控制，明顯提升攻擊時手部可見動作幅度。
- **擬人比例再調整**：`CatAppearance.lua` 改為更接近人形的頭身比與四肢比例（減少過大頭、拉長手臂與腿部），改善「不像人」的視覺問題。
- **修改檔案**：`src/client/CombatClient.lua`、`src/server/CatAppearance.lua`、`src/shared/GameConfig.lua`
